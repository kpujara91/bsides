...
import styles from './ContactHeroBanner.module.scss';
    ...
    <>
      <section className={`relative ${styles.contactHeroBanner}`}>
        <div className="container">
          <Breadcrumb Data={OverviewContent?.contact?.Banner?.Breadcrumb} />
          <div className={`overflow-hidden ${styles.heroTitleWraper}`}>
            <h1 className={`font-nasalization text-center gradiant-title title-wrapper ${styles.heroTitleInner}`}>
              {OverviewContent?.contact?.Banner?.banerTitle}
              {/* <br /> <span className="relative"><FlipWords words={OverviewContent?.contact?.Banner?.bannerAnimationLoop} /></span> */}
            </h1>
          </div>
        </div>
        <Image src={OverviewContent?.contact?.Banner?.BGImage} width={1920} height={750} alt="bgimage" className={`absolute top-0 left-0 w-full h-full object-cover ${styles.bgImage}`}/>
      </section>
    </>
  );
};
