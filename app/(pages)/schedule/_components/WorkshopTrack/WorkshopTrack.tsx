...
import styles from "./WorkshopTrack.module.scss";
import { ScheduleContent } from "@/components/JsonFiles/schedule";
import Image from "next/image";

const WorkshopTrack: React.FC = () => {
      ...
      <div className="container">
        <div className={`${styles.titleWrapper} title-wrapper text-center`}>
          <h2 className={`title ${styles.title}`}>
            {ScheduleContent?.Schedule?.WorkshopTrackContent?.MainTitle}
          </h2>
          <div
            className={`flex items-center justify-center flex-wrap gap-5 ${styles.eventsTimeLine}`}
          >
            {/* {ScheduleContent?.Schedule?.WorkshopTrackContent?.LocationTitle && <p className="caption inline-block">
              {ScheduleContent?.Schedule?.WorkshopTrackContent?.LocationTitle}
            </p>} */}
            {/* {ScheduleContent?.Schedule?.WorkshopTrackContent?.Day1Date && (
          ...
          >
               <p className="caption inline-block">
            {selectedDay =="Day1" ? ScheduleContent?.Schedule?.WorkshopTrackContent?.Day1Date:ScheduleContent?.Schedule?.WorkshopTrackContent?.Day2Date}
            </p>
        ...
        <div className={styles.technicalCardsWrapper}>
          {selectedDay === "Day1" &&
            ScheduleContent?.Schedule?.WorkshopTrackContent?.TechnicalTalkListsDay1?.map(
              (lists, i) => {
                return (
                            ...
                            <div className={` ${styles.techCard}`}>
                              ...
                              >
                                <div className={`text-center shrink-0 order-1 ${styles.leftBadge}`}>
                                  <p className={`${styles.badge}`}>LOCATION</p>
          ...
          {selectedDay === "Day2" &&
            ScheduleContent?.Schedule?.WorkshopTrackContent?.TechnicalTalkListsDay2?.map(
              (lists, i) => {
                return (
                  <React.Fragment key={i}>
                    {/* {(lists?.ScheduleTitle || lists?.ScheduleTime) && (
