...
import styles from './ScheduleHeroBanner.module.scss';
    ...
    <>
      <section className={`relative ${styles.scheduleHeroBanner}`}>
        <div className="container">
          <Breadcrumb Data={ScheduleContent?.Schedule?.Banner?.Breadcrumb} />
          <div className={`overflow-hidden ${styles.heroTitleWraper}`}>
            <h1 className={`font-nasalization text-center gradiant-title title-wrapper ${styles.heroTitleInner}`}>
              {ScheduleContent?.Schedule?.Banner?.banerTitle}
            </h1>
          </div>
        </div>
        <Image src={ScheduleContent?.Schedule?.Banner?.BGImage} width={1920} height={750} alt="bgimage" className={`absolute top-0 left-0 w-full h-full object-cover ${styles.bgImage}`}/>
      </section>
    </>
  );
};

export default ScheduleHeroBanner;
