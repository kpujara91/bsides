    ...
    <>
      <section className={`relative ${styles.AwardsHeroBanner}`}>
        <div className="container">
          <Breadcrumb Data={ScholarshipContent?.Sponsors?.Banner?.Breadcrumb} />
          <div className={`overflow-hidden ${styles.heroTitleWraper}`}>
            <h1 className={`font-nasalization text-center gradiant-title title-wrapper ${styles.heroTitleInner}`}>
              {ScholarshipContent?.Sponsors?.Banner?.banerTitle1}
            </h1>
          </div>
        </div>
        <Image src={ScholarshipContent?.Sponsors?.Banner?.BGImage} width={1920} height={750} alt="bgimage" className={`absolute top-0 left-0 w-full h-full object-cover ${styles.bgImage}`}/>
      </section>
      {/* <div className="text-center">
              <a className={`inline-flex  items-center justify-center ${styles.bookNowBtn}`}  href={ScholarshipContent?.Sponsors?.Banner?.BookNowButtonURL} target="_blank">Scholarship</a>
            </div> */}
    </>
  );
};
