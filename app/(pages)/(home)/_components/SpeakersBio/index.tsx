...
const Index = () => {
  ...
  return (
    <div className={`container ${styles.speakersAttendingWrapper}`}>
      ...
      {HomeContent?.SpeakersBio?.SpeakersBioList?.map((card: Card, i: number) => {
          ...
          <div key={i} className={`py-2 pr-8 ${styles.ContentWrap}`}>
            ...
            >
              <div className='flex items-center gap-4'>
                <div className={`flex-shrink-0 ${styles.speakersImageWrapper}`}>
                    <Image src={card?.src} alt="Speaker" width={120} height={120}  />
                </div>
                <div>
                <span className={`font-nasalization ${styles.speakersTitle}`}>{card?.title}</span>
                <span className={`block ${styles.smallTitle}`}>{card?.description}</span>
                </div>
              </div>
              <svg
