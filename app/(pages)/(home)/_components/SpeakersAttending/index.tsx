            ...
            <section className={`ptb-160`} id="speaker">
                <div className="container">
                    ...
                    {/* <div className="title-wrapper text-center">
                        <span className={`h3 caption`}>
                            {HomeContent?.SpeakersAttending?.Caption}
                        </span>
                        <h2 className="title text-center">
                            {
                                HomeContent?.SpeakersAttending
                                    ?.OpemingSpeakerTitle
                            }
                        </h2>
                    </div> */}
                    {/* <div
                        className={`flex items-start justify-center flex-wrap gap-y-6 xl:gap-y-8 mb-160 speakerCardList`}
                    >
                    ...
                    <div
                        className={`flex items-start justify-center flex-wrap gap-y-6 xl:gap-y-8 mb-160 speakerCardList`}
                    >
                        {HomeContent?.SpeakersAttending?.ClosingKeynoteSpeakerList?.map(
                            (SpeakerList, i) => {
                                return (
                                    <React.Fragment key={i}>
                                        <div
                                            className={`relative w-1/2 sm:w-1/3 lg:w-1/4 speakerCard`}
                                        >
                                            <Image
                                                src={SpeakerList?.speakerImage}
                                                alt={SpeakerList?.speakerName}
                                                width={700}
                                                height={875}
                                                className={`speakerProfile`}
                                            />
                                            <div
                                                className={`absolute left-0 bottom-0 w-full p-2 sm:p-3 flex items-start xxxl:items-center justify-between flex-col xxxl:flex-row gap-2 speakerData`}
                                            >
                                                <div>
                                                    <h3
                                                        className={`font-nasalization speakerName`}
                                                    >
                                                        {
                                                    ...
                                                    <p
                                                        className={`font-nasalization speakerDesignation`}
                                                    >
                                                        {
                                                            SpeakerList?.speakerDesignation
                                                        }
                                                    </p>
                                                </div>
                                                <ul
                                                    className={`flex items-center gap-2 flex-shrink-0 socialMediaList speakersocialMediaList`}
                                                >
                                                    {SpeakerList?.speakerSocialMediaList?.map(
                                                        (list, j) => {
                                                            return (
                                                                <React.Fragment
                                                                    key={j}
                    ...
                    <div
                        className={`flex items-start flex-wrap gap-y-6 xl:gap-y-8 speakerCardList mb-160`}
                    >
                        {HomeContent?.SpeakersAttending?.TechnicalSpeakerList?.sort(
                            (a, b) => a.speakerName.localeCompare(b.speakerName)
                        )?.map((SpeakerList, i) => {
                            return (
                                <React.Fragment key={i}>
                                    <div
                    ...
                    <div className="title-wrapper text-center">
                        <h2 className="title text-center">
                            {HomeContent?.SpeakersAttending?.CxoSpeakersTitle}
                        </h2>
                    </div>
                    <div
                        className={`flex items-start flex-wrap gap-y-6 xl:gap-y-8 speakerCardList`}
                    >
                        {HomeContent?.SpeakersAttending?.CxoSpeakerList?.sort(
                            (a, b) => a.speakerName.localeCompare(b.speakerName)
                        )?.map((SpeakerList, i) => {
                            return (
                                <React.Fragment key={i}>
                    ...
                    {/* Govt.  Speakers */}
                    <div className="title-wrapper text-center mt-16">
                        <h2 className="title text-center">
                            {HomeContent?.SpeakersAttending?.GovtSpeakersTitle}
                        </h2>
                    </div>
                    <div
                        className={`flex items-start flex-wrap gap-y-6 xl:gap-y-8 speakerCardList`}
                    >
                        {HomeContent?.SpeakersAttending?.GovtSpeakerList?.sort(
                            (a, b) => a.speakerName.localeCompare(b.speakerName)
                        )?.map((SpeakerList, i) => {
                            return (
                                <React.Fragment key={i}>
